...
function createMainWindow() {
	...
	mainWebview.onIpcMessage((data) => {
		...
		// console.log(data);
		// console.log(`Received IPC: ${JSON.stringify({ type: data.type, payload: data.payload })}`);
		...
		try {
			if (data.type === "launcherToggle") {
				...
				const result = dialog.showOpenDialogSync(mainWindow, {
					title: "Select NAT-bridge Configuration File",
					properties: ["openFile"],
...
app.bind((event) => {
	...
	if (event.event === WebviewApplicationEvent.ApplicationWillQuit) {
		if (currentProcess && !currentProcess.killed) {
			if (os.platform() === "win32") {
				killNatBridgeCmdWindows();
			} else {
